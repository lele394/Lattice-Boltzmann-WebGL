...
void main() {
    ...
    // Example 2: Vertical obstacle in center (uncomment to add)
    // float obstacleX = u_res.x * 0.5;
    // float obstacleWidth = 20.0;
    // if (abs(pixelCoord.x - obstacleX) < obstacleWidth && pixelCoord.y > u_res.y * 0.3 && pixelCoord.y < u_res.y * 0.7) {
    //     isWall = 1.0;
    // }
    
    // Wall format: vec4(isWall, velocityX, velocityY, reserved)
    // For static walls, velocity is 0
